...
- 🚀 **安全防護與效能**：
  ...
  - 記憶快取與 TTL 時效（Cache TTL）：在快取時效內重複需要資料時直接讀取快取，不重複呼叫 API；時效過期後隨下一次主動查詢更新。
...
- **🔄 立即更新名額 按鈕**
  - 點擊後會立即清空記憶體快取、中斷背景舊請求，並以最高優先級重新查詢當前頁面所有課程名額，同時在頁面右上角顯示非阻擋式的刷新進度指示。
...
* **超過 3 秒後**：
  * 舊快取資料自動被判定為過期失效。
  * **【重要】「快取失效」本身不會自動觸發 API 請求**。
* **重新取得最新名額的時機**：
  * 必須等到**下一次實際查詢被觸發**時，系統才會向學校 API 重新索取最新名額。
  * 觸發時機包含：
    1. 點擊彈出視窗內的 **「🔄 立即更新名額」** 按鈕（手動強制刷新）。
    2. 在選課系統中切換分頁、重新搜尋或變更每頁筆數（觸發頁面重新掃描）。
    3. 在 Hover 模式下，滑鼠再次移至課程的人數圖示上。

### 3. 為何採用此設計？

1. **避免重複請求學校 API**：防止在切換分頁或短時間重複渲染時頻繁呼叫同一端點。
2. **降低學校系統負載**：選課尖峰期間校方伺服器承載龐大流量，快取能防止不必要的請求堵塞通道。
3. **確保需要時能迅速取得新資料**：在熱門課程釋出名額的關鍵階段，設定為 3 秒快取後，只要主動觸發查詢（例如點擊「立即更新名額」或換頁），即可確保不會被長達數十秒或數分鐘的舊快取綁住，能夠即時獲取學校端最新異動後的名額。
...
**答**：這是正常的！「快取 3 秒」是 **Cache TTL（資料有效期限）**，而非自動定時刷新器。  
若需要即時查看名額是否變動，請在想確認的時間點點擊擴充功能選單中的 **「🔄 立即更新名額」**，或在選課系統中點擊下一頁／重新查詢，系統便會立即向學校獲取當下的最新名額。
