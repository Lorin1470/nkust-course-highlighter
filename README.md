...
* **重新取得最新名額的時機**：
  * 必須等到**下一次實際查詢被觸發**時，系統才會向學校 API 索取資料。
  * 常見情境包含：
    1. 點擊彈出視窗內的 **「🔄 立即更新名額」** 按鈕（手動強制刷新，會立即清空所有快取並強制重查當前畫面課程）。
    2. 在選課系統中切換分頁、重新搜尋或變更每頁筆數（可能觸發 DOM 重新掃描，**若該課程快取已過期才會重新向 API 查詢**；若快取未過期則仍直接沿用快取）。
    3. 在 Hover 模式下，滑鼠移至課程的人數圖示上（**若該課程快取已過期才會重新向 API 查詢**）。
...
若需要確保獲取最新名額變動，最直接的方式是在想確認的時間點點擊擴充功能選單中的 **「🔄 立即更新名額」**（會強制清空快取並立即重查）；若在選課系統中切換分頁或重新搜尋，擴充功能雖可能觸發重新掃描，但同樣**只有在該課程快取已過期**的情況下才會重新向學校發送 API 請求。
